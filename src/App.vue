...
function getUserData (text) {
  fetch(
    ...
    .then(data => {
      if (!userData.value) {
        userData.value = data
        showAlert(`خوش آمدید ${userData.value.name} عزیز`, 'success', '#22C55E')
      } else {
        userData.value = data
...
function getInstallments () {
  fetch('https://thermopay-174f7-default-rtdb.firebaseio.com/installments.json')
    .then(res => res.json())
    .then(data => {
...
function showReminders () {
  ...
  if (filterInstallments.length > 1) {
    ...
    showAlert(
      `شما امروز باید قسط های ${installmentsArray.join(',')} را پرداخت کنید`,
      'info',
      'rgb(68, 68, 240)',
      '#fff',
      'یاد آوری'
