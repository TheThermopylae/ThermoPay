<template>
  <div class="w-full h-screen fixed top-0 right-0 z-10">
    ...
    >
      <div class="text-2xl">
        <p>حذف کردن تمامی درآمد ها</p>
      </div>
      <p class="my-3">آیا میخواهید تمامی درآمد ها را حذف کنید؟</p>
      <div class="grid grid-cols-2 gap-5">
        <button
          class="btn btn-primary text-white rounded-full w-full"
          v-if="!loading"
          @click="removeIncomesFunc"
        >
          حذف کردن
        </button>
...
function removeIncomesFunc () {
  removeFunc(
    incomes,
    'https://thermopay-174f7-default-rtdb.firebaseio.com/incomes',
    getIncomes,
